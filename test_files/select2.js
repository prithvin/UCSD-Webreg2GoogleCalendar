...
(function ($, undefined) {
    ...
    AbstractSelect2 = clazz(Object, {
        ...
        moveHighlight: function (delta) {
			...
			var startFocusAfterKey = false;

			if ( -1 == index ) {  
				if ( delta < 1 ) { delta = 1 };
				startFocusAfterKey = true;
			}

            while ( startFocusAfterKey || ( index > -1 && index < choices.length ) ) {
            	index += delta;
				if ( index < 0 ) break;
        ...
        selectHighlighted: function (options) {
            ...
            if (data) {
				...
				if ( undefined != options && options.shiftKey ) {

					if ( ! shiftId1 ) {
						shiftId1 = highlighted.find("div").attr("id");
						var thisItem = $('.select2-result.select2-result-selectable.select2-highlighted');
						if ( undefined != thisItem && undefined != thisItem[0] )  {
							var thisLabel = thisItem['0'].innerHTML; 
							thisLabel = thisLabel.replace(/.*id="([^"]+)".*/, '$1');
							//$('#' + thisLabel).css('background-color' , 'grey');
							$('#' + thisLabel).css('background-color' , '#3875d7');
							$('#' + thisLabel).css('color' , 'white');
						}
						return;

					}else{ 
						var shiftId2 =  highlighted.find("div").attr("id");
						var shiftNum1 = shiftId1.replace(/.*-/, "");
						var shiftNum2 = shiftId2.replace(/.*-/, "");
						var shiftDataArr = []; 	shiftDataArr.length=0;

						for ( var i = Number(shiftNum1) ; i <= Number(shiftNum2) ; i++ ) {
							var leng = $("#select2-result-label-" + i).length; 
							shiftDataArr.push(
								$("#select2-result-label-" + i).closest('.select2-result').data("select2-data")
							);
						}

						shiftId1 = undefined;
						if ( shiftDataArr.length > 0 ) {
							this.onSelectShift( shiftDataArr , options);
                			this.close();
						}
					}
    ...
    MultiSelect2 = clazz(AbstractSelect2, {
        ...
        initContainer: function () {
            ...
            this.search.on("keydown", this.bind(function (e) {
                ...
                $(".select2-highlighted").mousedown(function(e){ e.preventDefault(); })
        ...
        // multi
        onSelectShift: function (data, options) {
        ...
        addSelectedChoice: function (dataArg) {

			var dataArr = []; dataArr.length=0;

			if ( undefined == dataArg.length ) { //object
				dataArr.push( dataArg);
			}else{
				dataArr = dataArg;
			}

			for ( var ix=0 ; ix < dataArr.length ; ix++ ) {
